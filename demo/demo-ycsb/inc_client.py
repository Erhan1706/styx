...
SAVE_DIR: str = f"{sys.argv[7]}/inc_ycsb_{messages_per_second}tps_{N_PARTITIONS}part_{current_time}"
...
def tps_for_second(sec_index: int) -> int:
    """Return the TPS for the given second of the run (0-based)."""
    step = sec_index // INTERVAL_SECONDS
    return messages_per_second + step * DELTA_TPS
...
def benchmark_runner(proc_num) -> dict[bytes, dict]:
    ...
    for sec in range(seconds):
        current_mps = tps_for_second(sec)
        batch_size = max(1, current_mps // sleeps_per_second)

        sec_start = timer()
        for i in range(current_mps):
            if i % batch_size == 0:
        ...
        if lps < 1:
            time.sleep(1 - lps)
        sec_end2 = timer()
        print(f'Current TPS: {current_mps}, latency per second: {sec_end2 - sec_start}')
